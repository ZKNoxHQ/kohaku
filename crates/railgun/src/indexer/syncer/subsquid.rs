...
/// ZKNOX fork: same query without `hasUnshield`, used if the indexer schema lacks the field.
...
impl SubsquidSyncer {
    ...
    async fn operations(
        ...
        match self.operations_with(OPERATIONS_QUERY, from, to).await {
            // ZKNOX fork: never let the extra field break txid sync. Without it, POI recovery
