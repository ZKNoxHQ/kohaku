...
pub struct TxidIndexer {
    ...
    /// ZKNOX fork: nullifiers of the registered accounts. Operations spending one of them are
...
pub(crate) struct TxidIndexerState {
    ...
    /// ZKNOX fork: operations that spend a watched nullifier.
    #[serde(default)]
...
impl TxidIndexer {
    ...
    /// ZKNOX fork: sets the nullifiers whose operations must be retained.
    pub fn watch_nullifiers(&mut self, nullifiers: std::collections::HashSet<U256>) {
        self.watched = nullifiers;
    }

    /// ZKNOX fork: retained operations of the registered accounts.
