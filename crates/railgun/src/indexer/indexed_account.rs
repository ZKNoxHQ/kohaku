...
pub struct IndexedAccountState {
    ...
    /// ZKNOX fork: notes this account has spent. Needed to rebuild a POI proof for an
    /// operation that was not built by this process (wallet restored from keys).
    #[serde(default)]
    pub spent: Vec<UtxoNote>,
    /// ZKNOX fork: outputs this account sent to other addresses.
    #[serde(default)]
...
impl IndexedAccount {
    ...
    /// ZKNOX fork: notes spent by this account.
    pub fn spent(&self) -> Vec<UtxoNote> {
        self.inner.spent.clone()
    }

    /// ZKNOX fork: outputs sent by this account to other addresses.
...
mod tests {
    ...
    /// ZKNOX fork: the sender recovers npk and value of an output addressed to someone else,
